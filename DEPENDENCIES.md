...
- **Version:** 25.1.0
- **Contract:** password hashing for the multi-user accounts (`pestilentia/security.py`): `hash_password`/`verify_password`/`needs_rehash` wrap `argon2.PasswordHasher` with argon2id defaults (RFC 9106 low-memory profile)
- **Licence:** MIT
- **Why chosen:** argon2id is the current OWASP first choice for password storage; argon2-cffi is the maintained reference binding. Chosen over passlib (dormant upstream, heavier surface) and bcrypt (weaker memory-hardness)
- **Alternatives considered:** passlib[argon2] (adds an abstraction layer the app does not need), bcrypt (no memory hardness), scrypt via stdlib (no maintained high-level API)
- **Transitive weight:** argon2-cffi-bindings + cffi + pycparser — small, CPU-only
